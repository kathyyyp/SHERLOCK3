# SHERLOCK - Medina - QC on merged SHERLOCK1&2&3 brush data and batch corrected with Combat_seq 
...
counts_brush_combat <- readRDS(file.path(combat.processed.data.dir, "counts_brush_combat.rds"))
...
pca_pipeline(clinical_file = clinical_brush, counts_file = counts_brush_merged, directory_name = "precombat_brush")
pca_pipeline(clinical_file = clinical_brush, counts_file = counts_brush_combat, directory_name = "postcombat_brush")
...
my_ensembl_gene_ids <- row.names(counts_brush_merged) #35,111 biomart
...
saveRDS(hgnc_symbols_db, file.path(medina.data.dir,"hgnc_symbols_db.rds"))
write.csv(hgnc_symbols_db, file.path(medina.data.dir,"hgnc_symbols_db.csv"))
...
unique_patients_all <- clinical_brush[match(unique(clinical_brush$Study.ID), clinical_brush$Study.ID),]
...
patient_demographics <- rbind(as.numeric(table(clinical_brush$classification)),patient_demographics)
...
write.csv(dup_rows, file.path(qc.dir, "duplicated_patients.csv"))

dup_rows_counts <- counts_brush_merged[1:10,row.names(dup_rows)]
write.csv(dup_rows_counts, file.path(qc.dir, "duplicated_patients_counts.csv"))
...
colsums_persample=cbind(precombat = colSums(counts_brush_merged), 
                        postcombat = colSums(counts_brush_combat))
write.csv(colsums_persample, file = file.path(qc.dir,"sherlock_all_colsums.csv"))
# lowest is 551,861 precombat and 558,608 postcombat

## Number of detected genes ------------------------------------------------------------------------------
genes_per_sample = cbind(precombat = colSums(counts_brush_merged >0), 
                         postcombat = colSums(counts_brush_combat >0))
write.csv(genes_per_sample, file = file.path(qc.dir,"sherlock_all_colsums_genes_detec_per_sample.csv"))
#lowest was 16,179
...
# ================================================================================== #
# 6. Troubleshooting batch correction - poor results ================================
# ================================================================================== #

libsize <- colSums(counts_brush_merged)
by_batch <- tapply(X = libsize, INDEX = clinical_brush$batch, FUN = summary) #index is a factor used to split X into groups. must be same length as X, FUN is the function i want to apply to each group
by_batch

median_lib <- tapply(X = libsize, INDEX = clinical_brush$batch, FUN =median)
median_lib
# fold-change of medians - largest / smallest
max(median_lib) / min(median_lib)

### FIGURE #####
pdf(file = file.path(qc.dir,"troubleshooting_plots.pdf"))
library(ggplot2)

clinical_brush$libsize <- colSums(counts_brush_merged)
clinical_brush$genes_per_sample <- colSums(counts_brush_merged >0)
clinical_brush$batch <- as.factor(clinical_brush$batch)

#boxplot - colsums per sample
ggplot(clinical_brush, aes(x=batch, 
                           y = libsize,
                           colour=batch)) +
  geom_boxplot() +
  theme_bw(base_size = 16) +
  labs(x="Batch", y="Library size (total reads)",
       title="Library size by batch")


#boxplot - genes detected per sample
ggplot(clinical_brush, aes(x= batch, 
                           y = genes_per_sample,
                           colour=batch)) +
  geom_boxplot() +
  theme_bw(base_size = 16) +
  labs(x="Batch", y="Genes detected per sample (reads > 0)",
       title="Genes detected per sample by batch")


# density plots

ggplot(clinical_brush, aes(x=log10(libsize), 
                           colour=batch,
                           fill = batch)) +
  geom_density(alpha=0.3, adjust=1) +
  theme_bw(base_size = 16) +
  labs(x="log10 total counts", y="Density",
...
#edgeR plotBCV
library(edgeR)
DGEL<- DGEList(counts=counts_brush_merged, group = clinical_brush[,"classification"])
keep <- filterByExpr(DGEL) #keeps genes that have count-per-million (CPM) above k in n samples, where k is determined by min.count and by the sample library sizes and n is determined by the design matrix.
DGEL<-DGEL[keep, , keep.lib.sizes=FALSE] 
DGEL<- calcNormFactors(DGEL,method = "TMM") #adjusts for library size, stores per-sample scaling factors here (DGEL$samples$norm.factors) but keeps raw counts the same
design <- model.matrix(~0 + classification + age + sex + smoking_status,
             data = clinical_brush)
DGEL <- estimateDisp(DGEL, design) #fits negative binomial dispersion
...
plotBCV(DGEL) #plot without keep
dev.off()


#inspecting for batch effects
DGEL<- DGEList(counts=counts_brush_merged, group = clinical_brush[,"classification"]) #edgeR only uses group for certain functions like fitlerbyExpr() or plotBCV()
DGEL<- calcNormFactors(DGEL,method = "TMM") #compute TMM normalisation to account for library size, note raw counts stay the same here
design <- model.matrix(~1, data = clinical_brush)
v <- voom(DGEL, design = design)#convert counts to logCPM, estiamtes mean-variance and assigns precision weights (makes data suitable for linear model)
fit <- lmFit(v, design) #fit model 
fit <- eBayes(fit) #stabilise variance estimates
logCPM_bc <- removeBatchEffect(x = v$E, batch=clinical_brush$batch) #x is a matrix containing log expression values

pca_res <- prcomp(t(as.data.frame(logCPM_bc)), scale. = TRUE, center = TRUE)
clinical_pca <- cbind(pca_res$x, clinical_brush)

png(file = file.path(qc.dir,"troubleshooting_plots_tmm_voom_pca.png"),width = 26, height = 23, units = "cm", res = 1200 )
# plot(pca2$x[,1], pca2$x[,2], col=as.factor(clinical_brush$batch))
...
# END OF THIS JOB 1760505878
# #################################################################################################################
# # Job details recorded by SLURM job epilog using sacct.                                                         #
# #################################################################################################################
# Resources consumed by job 1681714 for user umcg-kphung running on compute node nb-node-a02:
#   ================================================================================================================
#   JobID          Elapsed   AllocCPUS  AveCPU    ReqMem  MaxVMSize  MaxRSS    MaxDiskRead  MaxDiskWrite
# 1681714.batch  00:13:01  1          00:04:35          5219584K   4776272K  161.83M      146.28M
# #################################################################################################################
# 
