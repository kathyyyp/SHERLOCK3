...
for (sysmex_variable in colnames(clinical_brushbiopt[,619:ncol(clinical_brushbiopt)])){

sysmex_variable <- "Mono"
this_sysmex_variable <- sysmex_variable
...
diffexp_edgeR <- function(this.diffexp.dir, showEnsemblID = FALSE) {
  ...
  for (j in names(listofclinical)){
    ...
    clinical2 <- clinical2[intersect(row.names(clinical2), row.names(design)), ] #some patients are missing sysmex data
    ...
    # for (i in colnames(my.contrasts)){
      #specify coef = sysmex variable of interest
...
for (this_sysmex_variable in colnames(clinical_brushbiopt[,619:ncol(clinical_brushbiopt)])){
